...
export async function GET() {
  ...
  // Get vote counts per country in 24h and 2h windows
  ...
  for (const v of votes2h || []) {
    const s = map2.get(v.country_id) || { likes: 0, dislikes: 0 }
    if (v.vote_type === 'positive') s.likes++; else s.dislikes++
    map2.set(v.country_id, s)
  }

  // Get country names for top movers
  const topCountryIds = [...map24.entries()]
  ...
  const rising = [...trending]
    .sort((a, b) => b!.sentiment_delta - a!.sentiment_delta)
    .slice(0, 5)

  const falling = [...trending]
    .sort((a, b) => a!.sentiment_delta - b!.sentiment_delta)
    .slice(0, 5)
