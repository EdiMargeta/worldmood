...
export default function ActivityFeed() {
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          // Fetch country details for the new vote
 	    ...
 	    .single() as any
          ...
          const pool = messages[vote.vote_type as 'positive' | 'negative'] || messages.positive
  ...
  return (
    <div className="bg-[#0d0d1a] border border-white/10 rounded-2xl overflow-hidden">
      ...
      <div
        ref={listRef}
        className="overflow-y-auto max-h-[400px] divide-y divide-white/5"
      >
        {loading ? (
          ...
          items.map((item, idx) => (
            <div
              ...
              className={`flex items-start gap-3 px-4 py-3 transition-all ${
                idx === 0 ? 'bg-white/5 animate-in fade-in duration-500' : 'hover:bg-white/3'
              }`}
            >
